...
plot(febData$timestamp,febData$Global_Active_Power, ylab = "Global Active Power", type = "l")
plot(febData$timestamp, febData$Voltage, xlab = "datetime", ylab = "Voltage", type = "l")
plot(febData$timestamp, febData$Sub_metering_1, xlab= "", ylab = "Energy sub metering" , ylim = c(0,30))
...
legend("topright", c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), lty = 1,lwd = 2.5, col = c("black", "red", "blue"))
plot(febData$timestamp, febData$Global_reactive_power, xlab = "datetime", ylab = "Global_reactive_power", type = "l", bty="n", cex=.5)
dev.off()
